...
// ======================
// ✅ Login avec token depuis l'environnement
// ======================
const token = process.env.TOKEN || config.token
if (!token || token === 'YOUR_TOKEN_HERE') {
    console.error("Erreur : la variable d'environnement TOKEN n'est pas définie ou est invalide !")
    process.exit(1)
}
console.log("Token utilisé (5 premiers caractères) :", token.substring(0,5))
...
bot.on('messageCreate', function (message) {
  ...
  const command = message.content.split(config.prefix)[1].split(' ')[0],
    ...
    // @ts-expect-error This is fine, it's juste how the import works
