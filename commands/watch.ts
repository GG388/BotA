...
async function run(bot: Client, message: Message, args: string[]) {
  ...
  switch (processed.type) {
    ...
    case 'category': {
      ...
      addWatchlistItem({
        ...
        pricePercentage: processed.pricePercentage as number,
        difference: processed.difference as number,
        symbol: results.list?.[0]?.symbol || '€',   // ← correction ici
        name: results.name,
    ...
    case 'query': {
      ...
      addWatchlistItem({
        ...
        symbol: results[0]?.symbol || '€'   // ← correction ici aussi
      })
  ...
  // Ajout des infos supplémentaires sur les seuils
  if (processed.priceLimit) {
    response += `\nPrice must be below ${processed.priceLimit}€`
  }
  ...
  if (processed.difference) {
    response += `\nPrice must be more than ${processed.difference}€ off previous detected price`
  }
